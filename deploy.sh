...
# === Выдача прав на ioctl для камер ===
echo "🔌 === 6.5 Настройка прав доступа к оборудованию видеозахвата ==="
if ls /dev/video* >/dev/null 2>&1; then
    echo "🔓 Обнаружены устройства видеозахвата. Предоставление прав на чтение/запись..."
    sudo chmod 666 /dev/video*
    echo "✅ Права доступа к /dev/video* успешно установлены (chmod 666)."
else
    echo "⚠️  Предупреждение: Подключенные USB-камеры /dev/video* не обнаружены в системе!"
fi
